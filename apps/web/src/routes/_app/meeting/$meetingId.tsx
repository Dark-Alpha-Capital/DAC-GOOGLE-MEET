...
function MeetingDetailPage() {
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 px-4 py-12 sm:px-6">
      <Button variant="ghost" size="sm" asChild>
        <Link to="/">← Meetings</Link>
      </Button>
              ...
              {!run ? (
                ...
                <dl className="grid gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <dd className="mt-0.5 font-medium">
                      {formatWhen(run.leftAt, 'datetime')}
                    </dd>
                  </div>
                  <div>
              ...
              <Tabs defaultValue="transcript">
                ...
                <TabsContent value="transcript" className="mt-4">
                  {!run ? (
                    ...
                    <p className="text-sm text-muted-foreground">
                      Meeting in progress — transcript appears after the bot
                      leaves.
                    </p>
                  ) : run.transcriptText ? (
                    <pre className="max-h-[min(70vh,40rem)] overflow-auto whitespace-pre-wrap text-sm leading-relaxed">
                      {run.transcriptText}
                    </pre>
                  ) : run.status === 'failed' ? (
                    <Alert variant="destructive">
                  ...
                  {!run ? (
                    ...
                    <div className="space-y-3">
                      <p className="text-xs text-muted-foreground">
                        Everyone observed during the call (
                        {run.attendees.length}). Sync:{' '}
                      ...
                      <ol className="space-y-3">
                        {run.attendees.map((person, i) => (
                          <li
                            key={`${person.name}-${i}`}
                            className="border border-border bg-muted/40 px-3 py-2 text-sm"
                          >
                            <div className="flex gap-3">
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="mt-0.5 text-xs text-muted-foreground">
                                  first seen{' '}
                                  {formatWhen(person.firstSeenAt, 'datetime')}
                                  {person.lastSeenAt
